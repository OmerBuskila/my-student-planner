...
async function loadAllDataFromCloud() {
    ...
    try {
        const [coursesRes, itemsRes, pomoRes] = await Promise.all([
            supabaseClient.from('courses').select('*'),
            supabaseClient.from('items').select('*'),
            supabaseClient.from('pomo_history').select('*')
        ]);

        if (coursesRes.data) {
            courses = coursesRes.data.map(c => ({ id: c.id, name: c.name, color: c.color }));
        }
        if (itemsRes.data) {
            items = itemsRes.data.map(i => ({
                ...
                examSession: i.exam_session,
                createdAt: i.created_at
            }));
        }
        if (pomoRes.data) {
            pomoHistory = pomoRes.data.map(p => ({
                date: p.date,
...
async function syncItemToCloud(item) {
    ...
    await supabaseClient.from('items').upsert({
        ...
        exam_session: item.examSession || null,
...
async function syncPomoToCloud(pomo) {
    if (!currentUser) return;
    await supabaseClient.from('pomo_history').insert({
        user_id: currentUser.id,
        date: pomo.date,
        course_id: pomo.courseId || null,
...
document.getElementById('course-form').addEventListener('submit', (e) => {
    ...
    saveDataLocally();
    if(courseObj) syncCourseToCloud(courseObj);
...
function deleteCourse(courseId) {
    ...
    saveDataLocally();
    deleteCourseFromCloud(courseId);
...
function deleteCurrentExam() {
    ...
    const idToDelete = editingExamId;
    items = items.filter(i => i.id !== idToDelete);
    
    saveDataLocally();
    deleteItemFromCloud(idToDelete);
...
function finishPomodoro() {
    ...
    if (pomoMode === 'work') {
        ...
        pomoHistory.push(pomoObj);
        saveDataLocally();
        syncPomoToCloud(pomoObj);
        updatePomoStats();
    }
...
document.getElementById('item-form').addEventListener('submit', (e) => {
    ...
    if (editingItemId) {
        ...
        if (index !== -1) {
            data.completed = items[index].completed;
            items[index] = data;
        }
    } else {
        items.push(data);
...
function deleteCurrentItem() {
    if (editingItemId && confirm('למחוק פריט זה?')) {
        ...
        items = items.filter(t => t.id !== idToDelete);
